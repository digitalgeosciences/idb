...
export default function AuthorDetail() {
  ...
  const localAuthor = authors.find((a) => a.openAlexId === id);
  ...
  const authorWorks = useMemo(() => {
    ...
    return worksTable.filter((w) =>
      (w.allAuthorOpenAlexIds || []).includes(id),
    );
  }, [id]);
              ...
              <div className="flex gap-2">
                ...
                {localAuthor?.groupId && (
                  <Button
                    ...
                    size="sm"
                    className="w-fit px-2 text-xs"
                    onClick={() => navigate(`/group/${localAuthor.groupId}`)}
                  >
                    Back to program
                  </Button>
                )}
              ...
              <div className="flex flex-col gap-1 text-sm text-muted-foreground">
                {localAuthor && (
                  <div>
                    ...
                    {localAuthor.groupId ? (
                      <Link
                        to={`/group/${localAuthor.groupId}`}
                        className="text-primary hover:underline"
                      >
                        {localAuthor.groupName || localAuthor.groupId}
                      </Link>
                    ) : (
                      <span>{localAuthor.groupName}</span>
                    )}
                  </div>
                )}
                {localAuthor?.email && (
                ...
                <Table className="min-w-full">
                  ...
                  <TableBody>
                    {visibleWorks.map((work) => {
                      ...
                      const doiUrl =
                        rawDoi && rawDoi.toLowerCase().startsWith("http")
                          ? rawDoi
                          : rawDoi
                            ? `https://doi.org/${rawDoi.replace(/^doi:/i, "").trim()}`
                            : "";
